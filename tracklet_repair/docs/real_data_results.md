...
## Camera_03 Colab one-camera run, first 1000 frames

This run used `Warehouse_016`, `Camera_03`, and the first 1000 frames.
Detection used the ByteTrack/MOT17 fallback checkpoint with the one-class
YOLOX experiment because the trained AIC25 six-class detector checkpoint was
not available. ReID features were extracted with OSNet x1_0.

- Detector output: 4556 detections
- ReID output: 4556 feature files
- Raw BoT-SORT output: 3844 tracked objects across 40 track IDs and 976 frames
- Existing `single_camera_fix.py` output: 3961 objects across 20 track IDs and
  999 frames
...
| Metric | Before | After |
| --- | ---: | ---: |
| Total detections | 3844 | 3920 |
| Tracklets | 40 | 36 |
| Mean tracklet length | 96.10 | 108.89 |
| Median tracklet length | 19.50 | 27.50 |
| Short tracklets | 15 | 11 |
| Percent short tracklets | 37.50 | 30.56 |
| Tracklets with gaps | 21 | 12 |
| Total internal gaps | 46 | 17 |

- Interpolated detections: 76
- Merged tracklets: 4
- Merge map: `{9: 7, 10: 7, 25: 23, 32: 31}`

### Fixed fixed_Camera_03.json

| Metric | Before | After |
| --- | ---: | ---: |
| Total detections | 3961 | 3961 |
| Tracklets | 20 | 20 |
| Mean tracklet length | 198.05 | 198.05 |
| Median tracklet length | 83.00 | 83.00 |
| Short tracklets | 0 | 0 |
| Percent short tracklets | 0.00 | 0.00 |
| Tracklets with gaps | 0 | 0 |
| Total internal gaps | 0 | 0 |
...
### Camera_03 interpretation

On the raw output, internal gaps decreased from 46 to 17, tracklets with gaps
decreased from 21 to 12, median tracklet length increased from 19.50 to 27.50,
and short tracklets decreased from 15 to 11. Four fragmented tracklet pairs
were conservatively merged. The fixed output already had no internal gaps or
short tracklets, so the helper made no changes. This reinforces the Camera_02
pattern: the helper repairs raw fragmentation without over-modifying an
already stable fixed output.
