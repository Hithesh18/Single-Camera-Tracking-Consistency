...
class Tracker():
    ...
    def _init_parameters(self):
        ...
        self.parameters["split_cls"]:bool = False
        ...
        self.parameters["class_ids"]:dict = {
                            0 : "Person",
                            1 : "Forklift",
                            2 : "NovaCarter",
                            3 : "Transporter",
    ...
    def scpt_cls(self, tracking_dict):
        """
        This performs object tracking with single camera dataset.
        Most of code below are just copied from '20240214_OfflineTracking-Debug.ipynb' and tweaked few.
        """
        ...
        for time_section in range(last_frame // frame_period + 1): 
            ...
            # ---- STEP 1: Group objects by class ----
            class_serials_dict = defaultdict(list)
            for serial in serials:
                obj_class = tracking_dict[serial].get("Class", -1)  # fallback to -1 if missing
                class_serials_dict[obj_class].append(serial)

            # ---- STEP 2: Cluster each class group ----
            for obj_class, class_serials in class_serials_dict.items():
                if len(class_serials) == 0:
                    continue

                clusters = tracking_by_clustering(tracking_dict, class_serials, **self.parameters)

                # Offset cluster ids to avoid overlap
                clusters = [cluster + max_offlineid + 1 if cluster != -1 else -i for i, cluster in enumerate(clusters)]
                max_offlineid = max(clusters) if max(clusters) > 0 else max_offlineid

                if time_section == 0:
                    for serial, cluster in zip(class_serials, clusters):
                        tracking_dict[serial]["OfflineID"] = int(cluster)
                else:
                    past_serials = [
                                        serial for serial in time_section_serial_dict[time_section - 1]
                                        if tracking_dict[serial].get("Class", -1) == obj_class
                                    ]
                    tracking_dict = associate_cluster_between_period(
                        tracking_dict, clusters, class_serials, past_serials, **self.parameters
                    )
    ...
    def mcpt(self,scene_id, json_dir,out_dir):
        ...
        for json_file in json_files:
            ...
            # camera_id = json_file.split("_")[1] + "_" + json_file.split("_")[2]
            camera_id = extract_camera_tag(json_file)
...
def extract_camera_tag(filename):
    match = re.search(r'(Camera(?:_\d+)?)(?!\d)', filename)
    if match:
