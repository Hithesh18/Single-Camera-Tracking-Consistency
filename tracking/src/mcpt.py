...
def create_similarity_matrix_mcpt(representative_nodes,**kwargs):
    ...
    split_cls = kwargs.get("split_cls",False)
    feature_stack = None
    for camera_id in representative_nodes:
        ...
        for local_id in tmp_representative_nodes:
            ...
            if not split_cls:
                ...
                if representative_selection_method == "keypoint" and (representative_node['class_name'] == 'Person'
                                                                      or representative_node['class_name'] == 'FourierGR1T2'
                                                                       or representative_node['class_name'] == 'AgilityDigit' ):
                    score = representative_node["score"]
                    if score > keypoint_condition_th:
                        continue
...
def decide_representative_nodes(tracking_results,out_dir,scene_id,**kwargs):
    ...
    for camera_id in tracking_results:
        ...
        if representative_selection_method == "keypoint":
            keypoints_results = pose.PoseKeypoints(f"Pose/{str(scene_id).zfill(3)}/{str(camera_id).zfill(4)}/{str(camera_id).zfill(4)}_out_keypoint.json")
        ...
        if not split_cls:
            ...
            for local_id in local_id_serials_dict:
                ...
                for s in serials:
                    class_id = tracking_dict[s].get("Class", -1)
                    class_counts[class_id] += 1

                # Optional: Print for debug
                print(f"LocalID {local_id} class distribution:")
                for cid, count in class_counts.items():
                    cname = class_ids.get(cid, f"Unknown({cid})")
                    # print(f"  - {cname}: {count} instances")
                most_common_class_id = max(class_counts.items(), key=lambda x: x[1])[0]
                class_name = class_ids.get(most_common_class_id)


                # Representative selection
                # if most repeated class is one of [Transporter, Forklift, NovaCarter] no need to go for keypoints.
                if representative_selection_method == "centrality" or class_name =="Forklift" or class_name =="NovaCarter" or class_name =="Transporter" :
                    serials, serial, feature = find_highest_centrality_node(tracking_dict, serials, epsilon=epsilon)
                    if serial is not None:
                        representative_node = {"serial": serial, "npy_path": tracking_dict[serial]["NpyPath"], "class_name":class_name}

                elif representative_selection_method == "keypoint":
                    serial, feature, score = find_high_confidence_keypoint_node(
                        tracking_dict, serials, keypoints_results, frame_serials_dict, keypoint_th=keypoint_th
                    )
                    representative_node = {"serial": serial, "score": score, "npy_path": tracking_dict[serial]["NpyPath"], "class_name":class_name}
                ...
                if serials !=[]:
                    representative_nodes[camera_id][local_id] = {"representative_node": representative_node, "all_serials": serials}
    json_path = os.path.join(out_dir, f"representative_nodes_{scene_id}.json")
...
def multi_camera_people_tracking(tracking_results, scene_id, json_dir, out_dir,**kwargs):
    ...
    if representative_nodes == None:
        representative_nodes = decide_representative_nodes(tracking_results,out_dir,scene_id,epsilon=epsilon,representative_selection_method=representative_selection_method,short_track_th=short_track_th,keypoint_th=keypoint_th, split_cls= split_cls , class_ids = class_ids)
    else:
    ...
    similarity_matrix = create_similarity_matrix_mcpt(representative_nodes,short_track_th=short_track_th,representative_selection_method=representative_selection_method,keypoint_condition_th=keypoint_condition_th, split_cls = split_cls)
    ...
    similarity_matrix = replace_similarity(representative_nodes,similarity_matrix,tracking_results,clusters,distance_th=distance_th, 
                                           ...
                                           representative_selection_method=representative_selection_method,split_cls=split_cls)
    ...
    camera_dict = create_camera_dict(representative_nodes,short_track_th = short_track_th,
                                     keypoint_condition_th=keypoint_condition_th, representative_selection_method=representative_selection_method, split_cls=split_cls)
    ...
    for camera_id in representative_nodes:
        ...
        for local_id in tmp_representative_nodes:
            ...
            if not split_cls:
                ...
                if representative_selection_method == "keypoint"  and (tmp_representative_nodes[local_id]["representative_node"]["class_name"] == 'Person'
                                                                      or tmp_representative_nodes[local_id]["representative_node"]["class_name"] == 'FourierGR1T2'
                                                                       or tmp_representative_nodes[local_id]["representative_node"]["class_name"] == 'AgilityDigit' ):
                    score = tmp_representative_nodes[local_id]["representative_node"]["score"]
                    if score > keypoint_condition_th:
                        continue
...
def replace_similarity(representative_nodes,similarity_matrix,tracking_results,clusters,**kwargs):
    ...
    if replace_similarity_by_wcoordinate:
        min_distance_matrix,max_distance_matrix,mean_distance_matrix = create_distance_matrix(representative_nodes,tracking_results, distance_type = distance_type,short_track_th =short_track_th, keypoint_condition_th = keypoint_condition_th,representative_selection_method =representative_selection_method, split_cls = split_cls)
