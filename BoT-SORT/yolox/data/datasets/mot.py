...
class MOTDataset(Dataset):
    ...
    def __init__(
        ...
        self.print_labels_stat()

    def print_labels_stat(self):
        """
        Print number of annotations per category based on currently sampled self.ids.
        """
        anno_ids = self.coco.getAnnIds(imgIds=self.ids, iscrowd=False)
        annotations = self.coco.loadAnns(anno_ids)

        category_counts = Counter([anno["category_id"] for anno in annotations])
        cats = self.coco.loadCats(self.coco.getCatIds())
        cat_id_to_name = {cat["id"]: cat["name"] for cat in cats}

        print("[✓] Annotation count per label (sampled subset):")
