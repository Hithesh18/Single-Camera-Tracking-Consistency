    ...
    def resume_train(self, model):
        if self.args.resume:
            ...
            if self.args.ckpt is not None:
                ...
                logger.info(f"Loaded {len(filtered_dict)} layers from pretrained model.")
                logger.info(f"{len(unloaded_keys)} layers were not loaded (likely head/classification layers).")
                for k in unloaded_keys:
                    logger.debug(f"Unloaded layer: {k}")
            self.start_epoch = 0
